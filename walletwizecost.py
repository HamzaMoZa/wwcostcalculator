...
def calculate_cost(num_users, cost_options):
    total_cost = sum(cost for key, option in cost_options.items() if eval(option['condition']) and option['enabled'])
...
if 'cost_options' not in st.session_state:
    st.session_state.cost_options = {
        "Database Paid": {"condition": "database_paid", "cost": 190, "enabled": database_paid},
        "Jira Paid": {"condition": "jira_paid", "cost": 110, "enabled": jira_paid},
        "Dwolla Upgrade": {"condition": "dwolla_upgrade", "cost": 200, "enabled": dwolla_upgrade}
    }
...
def add_cost_option():
    ...
    if st.button("Add Option", key='add_option'):
        st.session_state.cost_options[f"{option_type} - {st.session_state.new_option}"] = {"condition": condition, "cost": cost, "enabled": False}
# Add new cost option
...
for name, option in st.session_state.cost_options.items():
    option['enabled'] = st.checkbox(name, value=option['enabled'])
