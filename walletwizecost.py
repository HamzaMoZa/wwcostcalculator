...
def calculate_cost(num_users, cost_options):
    total_cost = sum(cost for key, (condition, cost) in cost_options.items() if eval(condition) and cost_options[key][2])
...
if 'cost_options' not in st.session_state:
    st.session_state.cost_options = {
        "Database Paid": ("database_paid", 190 if database_paid else 0, database_paid),
        "Jira Paid": ("jira_paid", 110 if jira_paid else 0, jira_paid),
        "Dwolla Upgrade": ("dwolla_upgrade", 200 if dwolla_upgrade else 0, dwolla_upgrade)
    }
def add_cost_option():
    ...
    if st.button("Add Option", key='add_option'):
        st.session_state.cost_options[f"{option_type} - {st.session_state.new_option}"] = (condition, cost, False)
with st.expander("Add New Cost Option"):
    ...
    add_cost_option()
for name, (condition, cost, enabled) in st.session_state.cost_options.items():
    st.session_state.cost_options[name][2] = st.checkbox(name, value=enabled)
